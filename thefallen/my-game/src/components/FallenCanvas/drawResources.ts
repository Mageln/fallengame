...
export const drawResources = (
  ...
  resources.forEach((res, index) => {
    ...
    if (
      res.icon instanceof HTMLImageElement &&
      res.icon.complete &&
      res.icon.naturalWidth > 0
    ) {
      ctx.drawImage(
        res.icon,
        iconX,
    ...
    // --- КНОПКА "+" ---
    if (
      icons.plus instanceof HTMLImageElement &&
      icons.plus.complete &&
      icons.plus.naturalWidth > 0
    ) {
      ctx.drawImage(icons.plus, buttonX, buttonY, buttonSize, buttonSize);
    } else {
      // Заглушка если иконка не загружена
